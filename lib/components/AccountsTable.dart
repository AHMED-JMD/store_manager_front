...
class _AccountsTableState extends State<AccountsTable> {
  int rowsPerPage = 5;
  late final source =
      AccountSource(deleteAccount: deleteAccount, refresh: refresh);
...
class AccountSource extends AdvancedDataTableSource<Account> {
  ...
  DataRow? getRow(int index) {
    ...
    return DataRow(cells: [
      ...
      DataCell(Text(
        moneyFormatter(currentRowData.account.toDouble()),
        style: const TextStyle(fontSize: 19, fontWeight: FontWeight.bold),
      )),
      DataCell(Row(
        children: [
          UpdateAccount(
            title: currentRowData.name,
  ...
  Future<RemoteDataSourceDetails<Account>> getNextPage(
    ...
    return RemoteDataSourceDetails(
        newdata.length,
        newdata
            .map((datas) => Account.fromJson(datas))
